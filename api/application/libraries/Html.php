...
class html {
                ...
                <?php if($id!=NULL){ ?>
                    ...
                    { 
                    	if(isset($dataList[0]->booking_id))
                    	{
                    		...
                    		<a href="javascript:void(0)" class="btn btn-primary btn-xs" onClick="<?php echo $temp_onclick; ?>;getEditRecBooking('<?php echo $data->$id; ?>');"><i class="fa fa-pencil" aria-hidden="true"></i></a> 
                    		<?php
                    	}
                    	else
                    	{
	                    	...
	                    	?>
	                        <a href="javascript:void(0)" class="btn btn-primary btn-xs" onClick="<?php echo $temp_onclick; ?>;getEditRec('<?php echo $data->$id; ?>');"><i class="fas fa-pen-fancy"></i></i></a> 
	                    <?php 
	                	}
                	}
                    else{ ?>
                        <a href="javascript:void(0)" class="btn btn-primary btn-xs disabled"><i class="fas fa-pen-fancy"></i></a> 
                    <?php } ?>
                    <?php if($delete==1){ ?>
                    ...
                    <?php }else{ ?>
                        <a href="javascript:void(0)" class="btn btn-danger btn-xs disabled"><i class="fas fa-trash" aria-hidden="true"></i></a>
                    <?php } ?>
                    </td>
                <?php } ?>
			  </tr>      
	...
	public function getCompanyDropdown($status=NULL,$selected=NULL,$where=NULL){
		...
		<option value="<?php echo $dropdown->comp_id; ?>" <?php if($dropdown->comp_id==$selected) echo ' selected'; ?>><?php echo $dropdown->comp_name; ?></option>
        <?php }
		}else{ ?>
	...
	public function getApiDropdown($status=NULL,$selected=NULL,$where=NULL){
		$whereSt = '1=1';
		if($status !== NULL) $whereSt .= " and status=".$status; 
		if($where != NULL)  $whereSt .= " and ".$where;
		$apiList = $this->CI->Acc_Model->getApiList($whereSt);
		if(count($apiList)>0){
		foreach($apiList as $dropdown){ ?>
			<option value="<?php echo $dropdown->api_id; ?>" <?php if($dropdown->api_id==$selected) echo ' selected'; ?>><?php echo $dropdown->api_name; ?></option>
        <?php }
		} else { ?>
	...
	public function getSelectDropdown($fieldname, $where = '', $selected = '', $tbl = '')
	{
		$where2  = " 1=1 AND ".$fieldname."!='' ";
		$where2 .= ($where != '') ? ("AND $where") : '';
		foreach ($this->CI->Acc_Model->getSelectDropdown($fieldname, $where2, $tbl)  as $key => $value) { ?>
